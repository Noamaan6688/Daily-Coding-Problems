...
public:
    ...
    int solve(int i1, int i2, int prev, vector<int>&a1, vector<int>&a2)
    {
        ...
        int notpick=1e7,pick=1e7;
        if(a1[i1]>prev) notpick=solve(i1+1,i2,a1[i1],a1,a2);
        i2=upper_bound(a2.begin(),a2.end(),prev)-a2.begin();
        if (i2<a2.size()) pick=1+solve(i1+1,i2+1,a2[i2],a1,a2);
    ...
    int makeArrayIncreasing(vector<int>& arr1, vector<int>& arr2) {
        ...
        if (solve(0,0,-1,arr1,arr2)>n1) return -1;
